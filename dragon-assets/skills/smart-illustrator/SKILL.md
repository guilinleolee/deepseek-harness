...
version: 2.0.0
description: |
  智能配图与 PPT 信息图生成器 V2。支持三种模式：(1) 文章配图模式 - Type × Style 二维配图系统；(2) PPT/Slides 模式 - 生成批量信息图；(3) Cover 模式 - 生成封面图。所有模式默认生成图片，`--prompt-only` 只输出 prompt。触发词：配图、插图、PPT、slides、封面图、thumbnail、cover。
author: 天龙引擎团队 (合并 baoyu-article-illustrator)
created: 2026-02-26
updated: 2026-03-13
category: documentation

triggers:
  - "配图、插图、PPT、slides、封面图、thumbnail、cover、为文章配图、article illustrator"
---
...
## V2 新增：Type × Style 二维配图系统

基于 baoyu-article-illustrator 集成，支持更丰富的配图风格组合。
...
| **Style** | 视觉风格 | notion, warm, minimal, blueprint, watercolor, elegant |

自由组合：`--type infographic --style blueprint`

或使用预设：`--preset tech-explainer` → type + style 一键设置
...
| Style | 描述 | 最佳场景 |
|-------|------|---------|
| `notion` | 极简手绘线条 | 知识卡片、技术说明 |
| `warm` | 温馨友好 | 情感分享、生活方式 |
| `minimal` | 极简精致 | 高端品牌、专业内容 |
| `blueprint` | 蓝图技术风 | 架构、系统设计 |
| `watercolor` | 水彩艺术风 | 生活方式、艺术 |
| `elegant` | 优雅商务 | 商业报告、提案 |
| `sci-fi` | 科幻未来风 | 科技、创新 |
| `hand-drawn` | 手绘风格 | 教育、教程 |

### 预设系统

| Preset | Type | Style | 最佳场景 |
|--------|------|-------|---------|
| `tech-explainer` | infographic | blueprint | 技术说明、数据展示 |
| `story-time` | scene | warm | 故事叙述、情感内容 |
| `process-guide` | flowchart | notion | 流程指南、教程 |
| `compare-options` | comparison | minimal | 产品对比、选择 |
| `framework-viz` | framework | blueprint | 架构图、模型 |
| `history-timeline` | timeline | elegant | 历史演进、里程碑 |

### V2 使用方式

```bash
# 指定 Type 和 Style
/smart-illustrator article.md --type flowchart --style notion

# 使用预设
/smart-illustrator article.md --preset tech-explainer

# 指定配图密度
/smart-illustrator article.md --density balanced  # minimal/balanced/rich

# 混合类型（不同段落用不同类型）
/smart-illustrator article.md --type mixed
```
...
```
- [ ] Step 1: 分析内容类型（技术/教程/方法论/叙事）
- [ ] Step 2: 确定配图位置和类型
- [ ] Step 3: 选择风格（AskUserQuestion 确认）
- [ ] Step 4: 生成配图大纲（outline.md）
- [ ] Step 5: 生成图片
- [ ] Step 6: 插入文章并输出
```
...
- **v2.0.0** (2026-03-13) - 合并 baoyu-article-illustrator，新增 Type × Style 二维配图系统
- **v1.0.0** (2026-02-26) - 初始版本，支持三种配图模式
