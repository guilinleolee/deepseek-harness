...
description: 开发、维护、分发和验证 DeepSeek Harness (DSH) 插件的执行型 Skill。覆盖 host/client 形态判断、bundle/profile 契约、Service 与函数插件、工具、HTTP、持久化、slot、Conversation Node、客户端构建、HMR、GitHub 安装和真实组合验证。V3.2 起含 trajectory-debug 模式（devmom/dsh-trajectory-debug MIT · Stage 40 协同）。
metadata:
  version: "3.2.0"
  date: "2026-08-24"
  reference:
    - https://github.com/NanmiCoder/dsh-agent-teams
    - https://github.com/devmom/dsh-trajectory-debug
  upstream:
...
## 10. Trajectory-Debug 模式（⭐ V3.2 增量 · Stage 40 协同）
...
>
> **协同目标**：让任何新做的 DSH 插件**可以装上 trajectory-debug 即获得运行层可调试性**，而不是等出 bug 才排查。

### 10.1 三步安装 trajectory-debug

```sh
# Step 1：装插件（推荐从 npm 装：上游已发布 v0.1.0 全 5 包）
dsh plugin --profile web add dsh-trajectory-debug-bundle

# Step 2：验证 bundle 层
dsh web --dump-config
# → 应该看到 trajectory-debug-host / -remotes / ui-trajectory-debug 三行

# Step 3：装完重启 web
# 输入框 /trajectory + /perf 即可用
```
...
| 设计点 | 上游做法 | 借鉴建议 |
|---|---|---|
| **transport.ts · typert-无关 webserver RPC** | 不依赖 build-time codegen，走 `POST /api/trajectory-debug/rpc` | 任何频繁跨 host↔client 的 plugin 都可参考 |
| **bundle-client.mjs · esbuild 自构建** | 不走官方 tsdown，自写 esbuild 包装为 `__ModuleLoader__` | 官方 mainline 不支持时（如非常规 module） |
| **adapt.ts · 隔离层** | 唯一接触 DSH `SessionEvent` 的边界文件 | 任何 plugin 都应有此 firewall，免受 DSH rc 破坏性变更 |

### 10.3 测试范式（vitest 7 套件 / 56/56 PASS）

```
pnpm test
# → ✓ perf-analyzer.spec.ts (8)
# → ✓ diff-engine.spec.ts (10)
# → ✓ replay-engine.spec.ts (10)
# → ✓ trace-export.spec.ts (3)
# → ✓ model-tools.spec.ts (5)
# → ✓ transport.spec.ts (6)
# → ✓ provider.spec.ts (14)
```

**天龙 plugin 测试基线**：≥7 套件，覆盖 schema + boundary + transport + 端到端 + provider 生命周期。

### 10.4 trajectory-debug 测试代码片段模板

```ts
// 借鉴 trajectory-debug 的 provider.spec.ts 测试模式
import { Setup } from '@deepseek-ai/dsh-test-support/test-runtime'

describe('myPlugin', () => {
  it('initializes, registers effects, and disposes in order', async () => {
    const fx = await Setup.create({ bundles: ['myPlugin'] })
    expect(fx.pluginManager.list()).toContain('myPlugin')
    
    // 验证 effect 所有权
    expect(fx.effectTracker.hasUndisposed()).toBe(false)
    
    await fx.dispose()
    expect(fx.effectTracker.hasUndisposed()).toBe(false)  // 再次清理
  })
})
```

### 10.5 DON'T 护栏（trajectory-debug 增量）

- ❌ **不要**直接复制 `transport.ts`（依赖 DSH `Service` 注册生命周期，自己写需自管理）
- ❌ **不要**默认开 `enableModelTools`（每 step 多一次 LLM → token 翻倍；opt-in）
- ❌ **不要**让 trajectory-debug 干扰主线业务路由（仅做调试副线，路由决策与 40-01 MCP orchestrator 同级）
- ❌ **不要**让 trajectory JSON 当生产数据用（projection 才是稳定形态）

### 10.6 验证矩阵（trajectory-debug 模式）

| # | 必检项 | 期望 | 状态 |
|---|---|---|---|
| 1 | 5 包 npm install 退出码 0 | `pnpm -r build` exit 0 | ✅ Stage 40 D3 已验 |
| 2 | vitest 56/56 PASS | 与上游 README 声明一致 | ✅ Stage 40 D3 已验 |
| 3 | bundle-client.mjs 产出 lib/client.js | 17806 chars minified | ✅ Stage 40 D3 已验 |
| 4 | 真机装入 DSH `--dump-config` 出现插件层 | trajectory-debug-host/-remotes/-ui | ⏳ |
| 5 | /trajectory + /perf 斜杠命令注册 | 输入框可见 | ⏳ |
