...
export class JsonFileStore implements BridgeStore {
  ...
  addMessage(sessionId: string, role: string, content: string, _usage?: string | null): boolean {
    const msgs = this.loadMessages(sessionId);

    // Deduplicate consecutive user messages with same content (e.g., double-sent "在吗")
    if (role === 'user' && msgs.length > 0) {
      const lastMsg = msgs[msgs.length - 1];
      if (lastMsg.role === 'user' && lastMsg.content === content) {
        console.log(`[store] Skipping duplicate user message: "${content.substring(0, 30)}..."`);
