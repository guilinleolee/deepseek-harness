    ...
    while (this._running && !signal.aborted) {
      ...
      try {
        ...
        if (response.msgs && response.msgs.length > 0 && response.get_updates_buf) {
          ...
          // Non-batch mode: create a pending cursor entry so messages can be tracked
          // and acknowledged individually. Use rawOffset as the cursor - it will be
          // committed once all messages in this response are acknowledged.
          batchId = this.nextBatchId++;
          this.pendingCursors.set(batchId, {
            offsetKey,
            cursor: rawOffset,
          ...
          const nbBatch = this.pendingCursors.get(batchId);
          if (nbBatch) {
            nbBatch.sealed = true;
          }
        }
