...
github_hash: 72469e3ab8bd1a4dff23cdf391cc5aea9e81ee0c
last_updated: 2026-04-25
source_type: derived
github: 
repo: zarazhangrui/follow-builders
hash: 72469e3ab8bd1a4dff23cdf391cc5aea9e81ee0c
branch: main
date: 2026-03-20
note: Local version has feed updates and ONBOARDING_COMPLETE config handling
triggers: ["follow builders", "Follow Builders, Not Influencers"]
---
...
**Part A: Detect the current channel and get the target ID.**
...
**Part B: Create the cron job with explicit channel and target.**
```bash
openclaw cron add \
...
**Part C: Verify the cron job works by running it once immediately.**
```bash
openclaw cron list
...
Then run the full Content Delivery workflow (Steps 1–6 of the Content Delivery section below) right now, without
...
- `blogs` — blog posts with titles and summaries
- `prompts` — the remix instructions to follow
- `stats` — counts of podcast episodes, X builders, tweets, and blog posts
...
**Blog posts (process third):** The `blogs` array has recent posts. If present:
1. Summarize each post's `content` using `prompts.summarize_blogs`
2. Use `title`, `url`, and `name` from the JSON object
