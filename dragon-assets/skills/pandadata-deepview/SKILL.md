---
name: pandadata-deepview
description: Pandadata DeepView 期货数据分析技能包 - 席位博弈+期限结构+仓单库存+跨期套利
version: 1.0
category: finance
tags:
  - 期货
...
license: GPL-3.0
homepage: https://github.com/quantskills/skill-futures-deepview-analyst
requires:
  - pandadata-api
---

# 🐼 Pandadata DeepView 期货分析技能包
...
`pandadata-deepview` 是一个 **Agent Skill**：它教会 AI Agent（Claude Code、Codex、Cursor 等）如何组合使用 Pandadata 的 **38 个期货 DeepView 接口** —— 席位净持仓保证金、建仓过程、盈亏排行、多空比、资金流、基差、期限结构、仓单库存、虚实盘比、现货利润、跨期套利 —— 对期货品种、主力合约和跨期结构做综合研判。

它解决的核心问题：DeepView 数据接口多、口径杂、单日快照大，直接让 Agent 裸调容易**编错参数、跨日拼接出伪信号、把推断说成事实**。本技能内置了请求分诊、小样本试调、同日对齐、证据链标注四道约束。

> 数据契约来自姊妹技能 [`pandadata-api`](https://github.com/quantskills/skill-pandadata-api)；本技能负责"怎么分析"，不负责"接口长什么样"。
...
cp -r pandadata-deepview ~/.claude/skills/pandadata-deepview

# Codex / OpenAI Agents（全局）
mkdir -p ~/.agents/skills
cp -r skill-pandadata-api ~/.agents/skills/pandadata-api
cp -r pandadata-deepview ~/.agents/skills/pandadata-deepview

# Cursor（项目内）
mkdir -p .cursor/skills .cursor/rules
cp -r skill-pandadata-api .cursor/skills/pandadata-api
cp -r pandadata-deepview .cursor/skills/pandadata-deepview
cp pandadata-deepview/agents/cursor-rule.mdc .cursor/rules/pandadata-deepview.mdc
```
...
本技能基于 [quantskills/skill-futures-deepview-analyst](https://github.com/quantskills/skill-futures-deepview-analyst) 构建，采用 GPL-3.0 许可证。

---

**版本**: 1.0 | **依赖**: pandadata-api
