...
// ============================================================================
// Content Curator Framework — Delivery Script
// ============================================================================
// Sends a digest to the user via their chosen delivery method.
// Supports: Telegram bot, Email (via Resend), Feishu, or stdout (default).
//
// Usage:
...
//
// The script reads delivery config from ~/.content-curator/config.json
// and API keys from ~/.content-curator/.env
//
// Delivery methods:
...
//   - "feishu": sends via Feishu API (reads credentials from openclaw.json)
...
async function sendEmail(text, apiKey, toEmail) {
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'Content Curator <digest@resend.dev>',
      to: [toEmail],
      subject: `Content Curator Digest — ${new Date().toLocaleDateString('en-US', {
...
// Sends the digest via Feishu IM using the Lark SDK.
// Reads credentials from openclaw.json — no extra config needed.
async function sendFeishu(text, openclawConfig) {
  const require2 = createRequire(import.meta.url);
  const Lark = require2('D:/NODE/npm-global/node_modules/openclaw/node_modules/@larksuiteoapi/node-sdk');
  const feishu = openclawConfig?.channels?.feishu;
  if (!feishu?.accounts) throw new Error('Feishu account not found in openclaw.json');

  const accountKey = Object.keys(feishu.accounts)[0];
  const creds = feishu.accounts[accountKey];
  if (!creds?.appId || !creds?.appSecret) throw new Error('Feishu appId/appSecret not found');
  ...
  // receive_id: the Feishu user/group open_id from allowFrom list
  const receiveId = feishu.allowFrom?.[0];
  if (!receiveId) throw new Error('Feishu allowFrom not configured in openclaw.json');

  // Feishu "post" message format — content is a string (already JSON-stringified),
  // zh_cn.content is [[{ tag, text }]] (array of arrays of tag objects)
  ...
  if (res?.code !== 0) {
    throw new Error(`Feishu API error: ${res?.msg || JSON.stringify(res)}`);
  }
}
  ...
  try {
    switch (delivery.method) {
      ...
      case 'feishu': {
        ...
        if (!existsSync(openclawPath)) throw new Error('openclaw.json not found at ~/.openclaw/openclaw.json');
        const openclawConfig = JSON.parse(await readFile(openclawPath, 'utf-8'));
        await sendFeishu(digestText, openclawConfig);
        console.log(JSON.stringify({
          status: 'ok',
          method: 'feishu',
