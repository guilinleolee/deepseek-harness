...
export const FLASH_COST_CONFIG: Required<CostConfig> = {
  inputPerMillion: 1,
...
export const PRO_COST_CONFIG: Required<CostConfig> = {
  inputPerMillion: 3,
...
export const DEFAULT_COST_CONFIG: Required<CostConfig> = FLASH_COST_CONFIG

/** Resolve a partial cost config against the defaults. */
export function resolveCostConfig(config: CostConfig = {}): Required<CostConfig> {
  return {
...
export function costOfTokens(count: number, perMillion: number): number {
  if (count <= 0 || !Number.isFinite(count)) return 0
  return (count / 1_000_000) * perMillion
}

/** Total cost of one provider usage record. */
export function costOfUsage(usage: TokenUsage, config: Required<CostConfig>): number {
...
export function breakdownOfUsage(usage: TokenUsage, config: Required<CostConfig>): CostBreakdown {
