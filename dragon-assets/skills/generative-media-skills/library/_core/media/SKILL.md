...
description: >
  生成式媒体技能 · media
  Use when user asks to generate media via muapi.ai
version: 1.0.0
...
当用户需要使用 media 功能时调用本 skill。

## L2: 调用方式

```bash
bash async-task-pattern/adapters/muapi.sh submit   --action "media" --payload '{"..."}' --timeout 300
```
