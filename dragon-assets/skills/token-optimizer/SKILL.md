...
triggers: ["token optimizer", "Token Optimizer"]
---
...
## terse_mode — caveman-terse 输出层压缩集成

`token-optimizer` 负责**路由层**节省，`caveman-terse` 负责**输出层**压缩。两者叠加实现 75-85% 双重节省。

### 双层节省架构

| 层级 | Skill | 节省率 | 优化对象 |
|------|-------|--------|---------|
| **路由层** | `token-optimizer` | ~50% | 模型选择 + 上下文压缩 |
| **输出层** | `caveman-terse` | ~65% | LLM 输出文本压缩 |
| **双重叠加** | — | **75-85%** | 路由 + 输出联合 |

### terse_mode 配置选项

```javascript
// ~/.claude/settings.json 或 skill 配置
{
  "terse_mode": {
    "intensity": "full",       // lite | full | ultra | wenyan-full | wenyan-lite
    "preserve_patterns": [
      "安全警告",
      "⚠️ WARNING",
      "❌ 不可逆操作"
    ...
    "output_targets": ["analysis_report", "code_comment", "cli_output"]
  }
}
```
...
| 模式 | 压缩率 | 适用场景 | 示例输出 |
|------|--------|---------|---------|
| `lite` | ~40% | 用户可见最终输出 | "Bug修复建议..." |
| `full` | ~65% | 内部日志/中间产物（默认） | "Bug修复建议，详见 L12" |
| `ultra` | ~80% | Token 成本敏感场景 | "Bug: L12, add guard" |
| `wenyan-lite` | ~40% | 中文文言风格 | "问题所在，行十二，加卫护可也。" |
| `wenyan-full` | ~65% | 中文文言风格 | "Bug在L12，加guard即可。" |

### 与 token-optimizer 的协同

```python
# model_router.py 中集成 terse_mode
def classify_task(task: str) -> dict:
    result = classify(task)
    # 路由层：选择最优模型
    model = select_model(result["intent"], result["complexity"])
    # 输出层：选择压缩强度
    terse = select_terse_mode(result["intent"], result["audience"])
    return {
        "model": model,
        "terse_mode": terse,
        "expected_savings": model_savings + terse_savings
    }

def select_terse_mode(intent: str, audience: str) -> str:
    if audience == "user":
        return "lite"       # 用户友好
    elif intent == "code_generation":
        return "off"        # 代码不压缩
    elif intent == "internal_analysis":
        return "full"       # 内部分析
    else:
        return "full"       # 默认
```
...
| 任务类型 | 正常输出 | Terse 输出 | 节省率 |
|---------|----------|-----------|--------|
| 解释 React 重渲染 bug | 1180 tokens | 159 tokens | **87%** |
| 修复 auth 中间件 | 704 tokens | 121 tokens | **83%** |
...
<!-- token-optimizer 集成 -->
token-optimizer intensity: ultra   # 自动选择 ultra 压缩
token-optimizer mode: default      # 关闭 terse
```
