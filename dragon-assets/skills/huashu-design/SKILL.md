...
# 花叔Design · Huashu-Design (天龙引擎 v1.0)

> **⚠️ 天龙视角 · 入口索引** — 下文是花叔原版 SKILL.md 全文（方法论 + 7 Phase 工作流 + 40 风格库）。本节是天龙引擎的**路由表 + 协同矩阵**，告诉你「什么时候用、用什么 references / scripts」。
>
> **优先级**：原 SKILL.md 讲「怎么做」，本节讲「天龙场景下怎么选 + 与其他 14 阶段 skill 怎么协作」。**先看本节定位，再读原文**。

---

## 🎯 天龙路由表 · 一张表定位入口

| 天龙场景 | 走 huashu-design 哪条线 | 关键 references | 关键 scripts | 协同天龙 skill |
|---------|------------------------|-----------------|-------------|---------------|
| **博客配图 / 公众号头图** | Step 1-4 快速原型 + Step 8 动画（可选） | `design-styles.md` · `typography.md` · `cinematic-patterns.md` | — | `gpt-image-2-style-library`（AIGC 备选）· `baoyu-cover-image` |
| **产品发布动画 / Launch Film** | Step 9.5 配音流水线 + `launch-film-director-notes.md`（必读 14KB）· `voiceover-pipeline.md`（18KB） | `render-video.js` · `render-video-seek.js` · `add-music.sh` · `tts-doubao.mjs` · `mix-voiceover.sh` | `voxcpm-tts-integration`（替代 tts-doubao.mjs · 9 中文方言 + 30 语言）· `voxcpm-voice-distillery` |
| **演示幻灯片（PPT/Deck）** | Step 1 deck 交付链 + `slide-decks.md`（36KB 必读）· `editable-pptx.md`（15KB）· `react-setup.md` | `export_deck_pptx.mjs` · `export_deck_pdf.mjs` · `html2pptx.js`（46KB 核心）· `gen_deck_thumbs.mjs` | `ppt-master V9.9`（天龙版 PPT 工具栈）· `qiaomu-mondo V1.1`（海报） |
| **App / iOS 高保真原型** | 「App / iOS 原型专属守则」（覆盖通用规则）+ `app-prototype.md`（10KB） | `assets/ios_frame.jsx` · `android_frame.jsx` · `browser_window.jsx` · `macos_window.jsx` · `design_canvas.jsx` | `13-01 设计师 V10.9`（含 huashu-animation 集成） |
| **信息图 / 数据可视化** | Step 7-8 + `cinematic-patterns.md`（11KB）+ `scene-templates.md`（7KB） | `verify.py`（验收） | `smart-illustrator V2.2`（章节配图）· `info-graphic-pro` |
| **专家评审 / 5 维打分** | Step 10 + `critique-guide.md`（9KB）· `verification.md`（4KB） | `verify.py` | `vadps2-scorer`（VADPS2 评分）· `quality-ratchet` |
| **品牌协议 / 多品牌支持** | 核心原则 #0 + `brand-asset-protocol.md`（15KB 必读）| `fetch_images.py` | `brand-asset-protocol`（天龙已有同名 skill，可强化）· `blogger-fingerprint-registry V2.0`（8 维博主指纹） |
| **Tweaks 实时调参（设计变体）** | `tweaks-system.md`（9KB）· `multi-perspective-parallel-case-study.md`（11KB） | — | `aetherviz-master`（互动教育可视化） |
| **需求模糊时** | 「Fallback 顾问模式」 Phase 1-5 → 3 套差异化方向 → 用户选 | `design-styles.md`（46KB · 40 风格库）| `w1-brand-protocol.html` · `w2-junior-designer.html` · `w3-fallback-advisor.html`（3 个工作流 demo） |

---

## 🔗 天龙 14 阶段协同矩阵（huashu-design 是设计底座）

```
输入（上游）         huashu-design（设计）        输出（下游）
─────────────       ─────────────────────       ─────────────
khazix-writer ────> 主题文案 ──> 高保真原型 ──> VoxCPM2 配音
laoli-writer  ────> 35-02 V13.3 老李风 ──> gpt-image-2 生图
aihot        ────> 热点主题 ──> deck 幻灯片 ──> multi-platform-publisher
voxcpm-voice-distillery ─> 博主 IP ──> brand-spec.md ──> gpt-image-2 图像 IP
ip-diagram-creator ──> 信息图 ──> smart-illustrator V2.2 + qiaomu-mondo V1.1
book-distiller  ───> 书籍金句 ──> 视觉卡片 ──> 公众号 / 小红书
baoyu-post-to-x ──> 内容分发 ──> 设计预览 ──> 35-02 社媒投放
                       │
                       └─> html2pptx.js ──> PPT 文件
                       └─> render-video.js ──> MP4 (BGM + SFX)
                       └─> export_deck_pdf.mjs ──> PDF deck
                       └─> narrate-pipeline.mjs ──> 配音视频
```

---

## 📦 资产清单（已下载 636KB · 文本方法论 + 工具脚本）

| 类别 | 文件数 | 大小 | 内容 |
|------|--------|------|------|
| **SKILL.md** | 1 | 56 KB | 花叔原版方法论（500 行 · 7 Phase 工作流 + 40 风格库） |
| **references/** | 26 | 416 KB | `design-styles` 46KB · `slide-decks` 36KB · `animation-pitfalls` 25KB · `voiceover-pipeline` 18KB · `typography` 18KB · `brand-asset-protocol` 15KB · `editable-pptx` 15KB · `launch-film-director-notes` 14KB · `video-export` 11KB · `hero-animation-case-study` 11KB · ... |
| **scripts/** | 15 | 164 KB | `html2pptx.js` 46KB（核心）· `render-video.js` 12KB · `narrate-pipeline.mjs` 10KB · `render-video-seek.js` 10KB · 4 个 `export_deck_*` · 3 个 shell（BGM / 配音 / 格式转换）· `fetch_images.py` · `tts-doubao.mjs` · `verify.py` |
| **未下载（按需补）** | — | ~217 MB | demos 21 个 HTML（参考实现）· assets BGM 6 个（24MB）+ SFX 45 个 + showcase 9 个 · React JSX 组件 8 个 |

---

## 🚫 不适用场景（huashu-design 边界）

- ❌ **生产级 Web App**（需要路由 / 状态管理 / 后端）→ 用 `frontend-design` / `frontend-patterns`
- ❌ **SEO 网站 / 营销页**（需 SSG / Lighthouse 95+）→ 用 `frontend-design`
- ❌ **数据驱动动态系统**（表格 / 表单 / CRUD）→ 用 `frontend-design` + 后端
- ✅ **适合**：高保真原型 / 幻灯片 / 动画 / 信息图 / 视觉评审 / 设计变体探索

---

## 🗂 references 26 个文件清单（按场景归类）

### 🎬 动画 / 视频 / 配音（9 个）
- `animation-best-practices.md` (21KB) — 动画最佳实践
- `animation-pitfalls.md` (25KB) — **必读** · 反 AI slop 动画陷阱
- `animations.md` (7KB) — 动画 API 速查
- `hero-animation-case-study.md` (11KB) — Hero 动画案例
- `launch-film-director-notes.md` (14KB) — **Launch Film 必读** · 万字 director notes
- `multi-perspective-parallel-case-study.md` (11KB) — 多视角并行案例
- `voiceover-pipeline.md` (18KB) — **配音流水线** · Step 9.5
- `video-export.md` (11KB) — 视频导出（MP4/GIF）
- `cinematic-patterns.md` (11KB) — 电影化模式

### 📊 幻灯片 / 信息图 / 原型（5 个）
- `slide-decks.md` (36KB) — **必读** · Deck 架构 / 1920×1080
- `editable-pptx.md` (15KB) — **可编辑 PPTX 导出**
- `app-prototype.md` (10KB) — **App/iOS 原型守则**
- `scene-templates.md` (7KB) — 场景模板
- `apple-gallery-showcase.md` (11KB) — Apple Gallery 风格

### 🎨 设计 / 评审 / Tweaks（6 个）
- `design-styles.md` (46KB) — **必读** · 40 种 HTML 原生风格库（网页 20 + PPT 20）
- `typography.md` (18KB) — 字体系统
- `brand-asset-protocol.md` (15KB) — **品牌资产协议** · 5 步硬流程
- `design-context.md` (7KB) — 设计上下文
- `critique-guide.md` (9KB) — 5 维评审
- `tweaks-system.md` (9KB) — Tweaks 实时调参
- `verification.md` (4KB) — 验收标准

### 🔧 工程 / 工作流 / 音频（6 个）
- `react-setup.md` (9KB) — React 技术红线
- `workflow.md` (7KB) — 标准工作流
- `content-guidelines.md` (8KB) — 内容规范
- `audio-design-rules.md` (10KB) — 音频设计规则
- `sfx-library.md` (10KB) — SFX 音效库索引

---

## 🛠 scripts 15 个文件清单（按功能归类）

### PPT/Deck 导出（4 个）
- `html2pptx.js` (46KB) — **核心** · HTML → 可编辑 PPTX
- `export_deck_pptx.mjs` (4KB) — Deck PPTX 导出
- `export_deck_pdf.mjs` (3KB) — Deck PDF 导出
- `export_deck_stage_pdf.mjs` (5KB) — Deck Stage PDF（带分阶段）
- `gen_deck_thumbs.mjs` (3KB) — Deck 缩略图

### 视频 / 配音（6 个）
- `render-video.js` (12KB) — **核心** · HTML 动画 → MP4
- `render-video-seek.js` (10KB) — 视频片段裁剪
- `narrate-pipeline.mjs` (10KB) — 配音流水线编排
- `add-music.sh` (4KB) — 嵌入 BGM
- `mix-voiceover.sh` (4KB) — 混音（人声+BGM+SFX）
- `render-narration.sh` (5KB) — 渲染旁白
- `tts-doubao.mjs` (7KB) — **火山引擎 TTS**（**天龙替换方案**：用 `voxcpm-tts-integration` · 9 中文方言 + 30 语言）

### 工具（3 个）
- `fetch_images.py` (4KB) — 抓取品牌资产图片
- `convert-formats.sh` (3KB) — 格式转换（HTML↔PNG↔MP4）
- `verify.py` (5KB) — 验收脚本

---

## 🔄 与子集 huashu-animation-engine 的关系

| 维度 | `huashu-animation-engine`（旧） | `huashu-design`（新 · 本文件） |
|------|-------------------------------|-------------------------------|
| **范围** | 仅动画子集（useTime / useSprite / interpolate）| 完整设计系统（6 大能力）|
| **文件** | 1 SKILL.md + 2 文件（6.5KB）| 1 SKILL.md + 26 references + 15 scripts（636KB）|
| **API 风格** | 自研 huashu-* API（花叔子集）| 原生 HTML/CSS/JS（无框架依赖）|
| **应用** | 13-01 设计师 V10.9 集成 | 全栈：原型/PPT/动画/信息图/评审/品牌协议 |
| **状态** | ⚠️ **DEPRECATED** — 标记 deprecated 头注，仍保留作为轻量备选 | ✅ **主用** |

**迁移建议**：天龙场景下优先用 `huashu-design`，仅当只需「动画代码片段生成」且不想读 53KB SKILL.md 时，回退到 `huashu-animation-engine`。

---

## 📝 来源与版本

- **来源**：[alchaincyf/huashu-design](https://github.com/alchaincyf/huashu-design) master @ 2026-07-17
- **Stars**：21,565 ⭐（仓库描述：HTML-native design skill · Agent-agnostic · MIT License）
- **协议**：MIT（个人和商用都免费，自 2026-05-14 起）
- **集成版本**：天龙引擎阶段 15 V1.0
- **保留度**：100% — 原 SKILL.md 全文 + 全部 26 references + 全部 15 scripts 不做任何修改，天龙仅在入口加路由层

---

# ⬇️ 以下是花叔原版 SKILL.md 全文（保持原貌，仅添加上述天龙入口层） ⬇️
