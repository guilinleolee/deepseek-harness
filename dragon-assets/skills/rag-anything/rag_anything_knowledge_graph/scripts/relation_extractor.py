...
class RelationExtractor:
    ...
    async def extract_from_entity_pair(
        ...
        if context:
            ...
            # 检测 "X is short for Y" 或 "X stands for Y" 模式
            abbreviation_patterns = [
                rf"{re.escape(name1)}\s+is\s+(?:short\s+for|abbreviated\s+as)\s+{re.escape(name2)}",
                rf"{re.escape(name1)}\s+stands\s+for\s+{re.escape(name2)}",
                rf"{re.escape(name2)}\s+is\s+(?:short\s+for|abbreviated\s+as)\s+{re.escape(name1)}",
                rf"{re.escape(name2)}\s+stands\s+for\s+{re.escape(name1)}",
            ]

            for pattern in abbreviation_patterns:
                if re.search(pattern, context, re.IGNORECASE):
                    # 确定 source 和 target
                    if name1 in pattern.split(r"\s+is\s+")[0] if "is" in pattern else name2:
